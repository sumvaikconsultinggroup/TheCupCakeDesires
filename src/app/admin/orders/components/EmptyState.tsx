...
export default function EmptyState({ hasFilters, onClearFilters, message }: EmptyStateProps) {
  return (
    <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
      ...
      {hasFilters && onClearFilters && <button onClick={onClearFilters} className="px-4 py-2 text-sm font-medium text-white bg-[#2e1f15] hover:bg-[#0F0E5B] rounded-md transition-colors">Clear all filters</button>}
    </div>
  )
}
