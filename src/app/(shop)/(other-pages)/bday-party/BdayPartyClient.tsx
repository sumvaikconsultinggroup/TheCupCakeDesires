...
import type { BdayPartyCakeProduct } from '@/lib/bday-party-server'
...
const HERO_IMAGE =
  'https://images.unsplash.com/photo-1558301211-0d8c8ddee6ec?q=80&w=1200&auto=format&fit=crop&ixlib=rb-4.1.0'

export default function BdayPartyClient({ cakeProducts }: { cakeProducts: BdayPartyCakeProduct[] }) {
        ...
        <div className="mx-auto grid max-w-[1320px] grid-cols-1 gap-10 px-6 pt-12 pb-20 md:grid-cols-12 md:gap-16 md:px-10 md:pt-20 md:pb-28">
          ...
          >
            <div className="relative aspect-[4/5] w-full overflow-hidden rounded-2xl bg-rose-deep/20">
              <Image
                src={HERO_IMAGE}
                alt="Festive birthday cake with pink drip icing, cherries, and rainbow sprinkles"
                fill
                priority
        ...
        <div className="mx-auto max-w-[1320px] px-6 md:px-10">
          <div className="flex flex-wrap items-end justify-between gap-6">
            ...
            <p className="bake-body max-w-[42ch]">
              {cakeCount > 0
                ? 'Add a round cake or custom tier to your party — all baked to order from our Cakes collection. Eggless and vegan options available; just tell us at booking.'
                : 'Round cakes and custom tiers can be added to your party booking — ask us when you reserve your date.'}
            </p>
          </div>
            ...
            <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {cakeProducts.map((product, i) => (
                <CakeProductCard
                  ...
                  index={i}
                  badge={product.isVegan ? 'Vegan' : product.isEggless ? 'Eggless' : undefined}
                  badgeTone={product.isVegan ? 'mint' : product.isEggless ? 'gold' : 'rose'}
                />
              ))}
            </div>
          ) : (
            <div className="mt-12 rounded-2xl border border-line bg-cream px-8 py-12 text-center">
              <p className="bake-body text-cocoa-soft">Our cake menu is being refreshed — check back soon or call the studio.</p>
              <Link href="/collections/cakes" className="bake-btn bake-btn-ghost mt-6 inline-flex">
                Browse cakes <span aria-hidden>→</span>
              </Link>
            </div>
          )}
          ...
          <div className="mt-10 flex flex-wrap items-center justify-center gap-6">
            ...
            >
              See all cakes <span aria-hidden className="ml-1">→</span>
            </Link>
            <Link
