 ...
 *  - same-origin only
 *  - raster image types only (no SVG — it can carry active content)
 *  - 4 MB cap
 *  - fixed Cloudinary folder, so uploads can't be scattered around the account
 *  - original format preserved (the kitchen prints this artwork, so we don't
 *    want a lossy re-encode); only bounded in dimensions.
 */
const ALLOWED = ['image/png', 'image/jpeg', 'image/jpg', 'image/webp']
const MAX_BYTES = 4 * 1024 * 1024 // 4MB
const FOLDER = 'corporate-logos'
  ...
  try {
    if (!process.env.CLOUDINARY_CLOUD_NAME || !process.env.CLOUDINARY_API_KEY) {
      return NextResponse.json(
        { success: false, error: 'Logo uploads are not configured. Please contact us to send your artwork.' },
        { status: 503 }
      )
    }
    ...
    if (file.size > MAX_BYTES) {
      return NextResponse.json(
        { success: false, error: 'That file is too large — please keep your logo under 4MB.' },
        { status: 400 }
      )
    }
      ...
      const uploadStream = cloudinary.uploader.upload_stream(
        {
          folder: FOLDER,
          resource_type: 'image',
          // Keep the original format so transparency survives for printing.
